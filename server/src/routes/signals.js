...
import { runScanCycle } from '../scheduler/marketScanner.js';
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const signals = await Signal.find(filter).sort({ createdAt: -1 }).limit(100).lean();
...
router.post('/scan', async (_req, res, next) => {
  try {
    ...
    runScanCycle({ forceRun: true }).catch((err) =>
